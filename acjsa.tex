...
the ORAC-DR pipeline framework. In many cases these automatically
generated products were publication quality, and thanks to a
constantly upated calibration model, better than what an inexperienced
astronomer could be expected to achieve on their own. Moreover with
the advent of large bolometer arrays such as SCUBA-2, this data could
be processed in maps that resulted in image data that could be readily
understood by non-submm specialists.
...
the highest priority with the advent of the JCMT Survey Programme
using the SCUBA-2 and HARP/ACSIS instruments. Aside from the normal
desire to provide a uniformly reduced product to the survey teams, the
processing demands for this data required a non-trivial IT
infrastructure . Due to the iterative algorithms utilised by the
SCUBA-2 mapmpaker (SMURF), the more data could be co-processed, the
higher the fidelity of the solution, and circa 2008, machines with
speficiations such as 64GB of RAM were not readily available to the
typical JCMT observer. So there were intrinsic reasons to have a JCMT
Science Archive allowing the survey consortia to download the
processed products. But ultimately, usage of such an archive would be
dominated by JCMT users retrieving their own data, or after the
proprietary period ellapsed, other JCMT users working in the same
scientific areas.
...
Group was busy with supporting the entire non-hardware-controlling
...
which hosted the older JCMT data archive for the benefit of the
...
(bribes accepted for not entitling this CADC: Knighst in Shining
...
with a small amount of effort, CADC developed CAOM (the Common Archive
Object Model), an extensive and versatile data model that allowed any
data to be ingested in the CADC archive provided it could be
adequately mapped to it.
...
Meanwhile, CADC was working on the CANFAR (def.) project aiming to
...
Archive adopted a model of continuous release. As data was taken it
was pushed for reduction and was ingested at CADC on the say 24-hour
period it was observed. Thus high-quality science products were
published in the VO as soon as the PI had access to them. Moreover
with every major impovement in the data reduction software, data could
be re-processed and again immeditaly relesed.
