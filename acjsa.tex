...
\usepackage[pdftex,pdfpagemode={UseOutlines},bookmarks,bookmarksopen,draft,colorlinks,linkcolor={blue},citecolor={green},urlcolor={red}]{hyperref}
\usepackage{hypernat}
...
%% Text of abstract
  ...
  JSA is a successful example of a service designed to use {\color{red}Virtual Observatory (VO)}
...
{\color{red}The James Clerk Maxwell Telescope (JCMT) has collaborated with the Canadian Astronomy Data Centre (CADC) to
...
utilized by the SCUBA-2 map-maker {\color{red}
...
metadata. {\color{red}Even modern instruments on some older telescopes follow 
...
Do we use the IVOA Credential Delegation mechanism?  Are there other protocols that should be listed?
\begin{description}
\item[CAOM]: Common Object Observation Model -- Conceptually a superset of ObsCore, CAOM is a cross-archive data model implemented at the CADC to provide search and retrieval \item[DALI]: Data Access Layer Interface -- Implemented by the ``Data Web Service'' to provide access to the ``Archive Directory'' file storage resource. \url{???} 
\item[ObsCore]: Observation Data Model Core Components -- IVOA standard observation data model. \url{http://www.ivoa.net/documents/ObsCore/}
capabilities for all CADC archives.  The SIA and TAP services provide VO access to CAOM databases \url{http://www.cadc-ccda.hia-iha.nrc-cnrc.gc.ca/caom2/}
\item[SIA]: Simple Image Access Protocol -- The SIA server at the CADC can be used to discover, access, and retrieve multi-dimensional image datasets. \url{http://www.ivoa.net/documents/SIA/}
\item[TAP]: Table Access Protocol -- Used to query CAOM databases, translating obscure entities into CAOM entities transparently. \url{http://www.ivoa.net/documents/TAP/} 
\item[VOTable]: Virtual Observatory Table Format -- An XML standard for the interchange of data represented as a set of tables, TAP and AdvancedSearch can return query results to the user in VOTable format \url{http://www.ivoa.net/documents/VOTable/}
\end{description}
}
...
\begin{figure}[t]
\includegraphics[width=\columnwidth]{jsa_dataflow_2015}
...
\section{{\color{red} Extending VO for Radio Astronomy}}
...
MIRI;][]{2010SPIE.7731E..10W}.  {\color{red} The proposed
recommendation for SIA-2.0 (available at
\url{http://www.ivoa.net/documents/SIA/20141024/index.html}) 
...
TAP with CAOM (AdvancedSearch) & 1\,001\,618 \\
TAP with ObsCore & 889\,175\\
TAP querying for spectra & 215\,952\\
SIA Simple Image Access & 299\,793\\ \hline
\end{tabular}
\end{center}
\end{table}
...
pipeline environment is identical. {\color{red} Similarly, the ingestion
...
to understand than anything that has been available previously. {\color{red}
...
current database schema can be found at \url{http://www.cadc-ccda.hia-iha.nrc-cnrc.gc.ca/caom2/}.} 
